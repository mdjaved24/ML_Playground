...
function Dashboard() {
  ...
  const metrics = useMemo(() => {
    ...
    const recentTrainTime = recentCount
      ? recent.reduce((sum, m) => sum + (m.training_time || 0), 0) / recentCount
      : 0;
    const timeTrend = avgTrainingTime
      ? ((recentTrainTime - avgTrainingTime) / avgTrainingTime) * 100
      : 0;

    // Active model percentage
    const activePercent = totalCount
      ? (dashboardData.active_models / totalCount) * 100
      : 0;
