...
function Dashboard() {
  ...
  // Calculate metrics only when dashboardData is available
// ✅ useMemo is always called
  const metrics = useMemo(() => {
    ...
    const avgAccuracy = dashboardData.avg_accuracy || 0;
    const avgTrainingTime = dashboardData.avg_training_time || 0;
    const activeModels = dashboardData.active_models || 0;

    const recentCount = recent.length;

    const recentAvgAcc = recentCount
      ? recent.reduce((sum, m) => sum + (m.accuracy || 0), 0) / recentCount
      : 0;
    const accTrend = avgAccuracy
      ? ((recentAvgAcc - avgAccuracy) / avgAccuracy) * 100
      : 0;
    const recentHasTrainingTime = recent.some(m => m.training_time != null);

    const recentTrainTime = recentHasTrainingTime
      ? recent.reduce((sum, m) => sum + (m.training_time || 0), 0) / recent.length
      : null;

    const timeTrend = recentTrainTime != null && avgTrainingTime != null
      ? (recentTrainTime - avgTrainingTime)
      : null;
      
    const activePercent = totalCount
      ? (activeModels / totalCount) * 100
      : 0;
    ...
    return [
      ...
      },
      {
        ...
        trend: `${accTrend >= 0 ? '+' : ''}${accTrend.toFixed(2)}%`,
      },
      {
        title: "Active Models",
        value: activeModels,
      ...
      },
      {
        ...
        value: `${avgTrainingTime.toFixed(2)}s`,
        icon: <FiClock className="text-2xl text-orange-500" />,
        description: "Per model",
        trend: `${timeTrend >= 0 ? '+' : ''}${timeTrend.toFixed(2)}%`,
      },
    ];
  }, [dashboardData]);
