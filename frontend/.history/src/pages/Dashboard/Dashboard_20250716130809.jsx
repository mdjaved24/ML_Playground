...
function Dashboard() {
 ...
 const metrics = useMemo(() => {
  ...
  const avgTrainingTimeRaw = dashboardData.avg_training_time;
  ...
  ? recent.reduce((sum, m) => {
      const time = parseFloat(m.training_time ?? 0);
      return time > 0 ? sum + time : sum;
    }, 0) / 
    recent.filter(m => parseFloat(m.training_time ?? 0) > 0).length
  : null;

// Handle case where all training times are 0
const validRecentTrainTime = recentTrainTime && isFinite(recentTrainTime) ? recentTrainTime : null;

const timeTrend =
  avgTrainingTime != null && validRecentTrainTime != null
    ? validRecentTrainTime - avgTrainingTime
    : null;
  // ✅ Active Model %
