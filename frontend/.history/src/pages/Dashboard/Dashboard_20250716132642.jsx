...
const metrics = useMemo(() => {
  ...
  // Parse averages
  const avgAccuracy = parseFloat(dashboardData.avg_accuracy || 0) || null;
  ...
  // Avg Accuracy (unchanged)
  const recentAvgAcc = recentCount
    ? recent.reduce((sum, m) => sum + parseFloat(m.accuracy ?? 0), 0) / recentCount
    : null;

  const accTrend = avgAccuracy && recentAvgAcc 
    ? ((recentAvgAcc - avgAccuracy) / avgAccuracy) * 100 
    : null;

  // Avg Training Time (handle missing field)
  const recentTrainTime = recentCount && recent.every(m => 'training_time' in m)
    ? recent.reduce((sum, m) => sum + parseFloat(m.training_time || 0), 0) / recentCount
    : null;

  const timeTrend = avgTrainingTime && recentTrainTime 
  ...
  return [
    ...
    },
    {
      ...
      trend: recent.some(m => 'training_time' in m) 
        ? (timeTrend != null ? `${timeTrend >= 0 ? '+' : ''}${timeTrend.toFixed(2)}s` : '–')
        : 'N/A', // or '–'
    },
  ];
}, [dashboardData]);
