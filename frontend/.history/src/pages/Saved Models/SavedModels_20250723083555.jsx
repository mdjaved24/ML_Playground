...
const makePrediction = async () => {
  try {
    ...
    const orderedInputs = {};
    const orderedColumns = selectedModel.features.map(f => typeof f === 'string' ? f : f.name);
    
    // Ensure we have values for all expected features
    orderedColumns.forEach(col => {
      if (predictionInputs[col] === undefined) {
        throw new Error(`Missing value for feature: ${col}`);
      }
      orderedInputs[col] = predictionInputs[col];
    });
    ...
    const response = await axios.post(
      ...
      { 
        inputs: orderedInputs  // Send as dictionary
      },
      {
        headers: {
          'Authorization': `Bearer ${accessToken}`,
        ...
        <div className="min-h-screen bg-gray-50">
          <div className="max-w-7xl mx-auto px-4 py-8 sm:px-6 lg:px-8">
            <button 
              ...
              className="flex items-center text-gray-600 hover:cursor-pointer hover:text-gray-900 mb-6"
            >
