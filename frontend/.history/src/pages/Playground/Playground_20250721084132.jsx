...
const handleChange = (e) => {
  ...
  // Update target column in form data
  setFormData(prev => ({
    ...prev,
    target_column: target
  }));

  // Extract values from previewData
  const targetValues = previewData.map(row => row[target]);

  // Clean and normalize values
  const normalizedValues = targetValues.map(val => String(val).trim());
  const uniqueValues = Array.from(new Set(normalizedValues));

  const uniqueLabels = new Set(targetValues);

if (uniqueLabels.size < 2) {
  toast.warning("⚠️ The selected target column contains only one class (e.g., only 0). Please choose a target with at least two classes.");
  return;
}

  // Handle binary classification (e.g., 0 and 1)
  const isBinary = uniqueValues.length === 2 && uniqueValues.every(val => ['0', '1'].includes(val));

  // Handle only one unique class – show error and exit early
  if (uniqueValues.length < 2) {
    toast.warning("⚠️ Model training will fail: The selected target column has only one unique value.");
    setTargetType(null);
    return;
  }

  // Set target type based on value pattern or column type
  if (isBinary) {
    setTargetType('classification');
  } else if (columnTypes[target] === 'numeric') {
  ...
  // Reset model type and parameters
  setFormData(prev => ({
    ...prev,
    model_type: '',
    parameters: {}
  ...
  const handleParameterChange = (paramName, value) => {
    setFormData(prev => ({
      ...
      parameters: {
        ...prev.parameters,
        [paramName]: value === '' ? '' : value  // Keep as empty string if cleared
      }
    }));
  };
  ...
  const handleBlur = (paramName) => {
    if (formData.parameters[paramName] === '') {
      setFormData(prev => ({
        ...prev,
        parameters: {
          ...prev.parameters,
          [paramName]: getCurrentModelParameters()[paramName].value
        }
      }));
    }
  };
                        ...
                        <div key={param}>
                          ...
                          {config.type === 'number' ? (
                            <input
                              ...
                              max={config.max}
                              step={config.step || 1}
                              placeholder={config.placeholder || String(config.value)} 
            ...
            <div className="bg-white rounded-xl shadow-md p-6 border border-[var(--color-gray-200)]">
              ...
              <div className="flex justify-end">
                <button
                  ...
                  className="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-gradient-to-r from-[var(--color-accent-500)] to-[var(--color-accent-600)] hover:from-[var(--color-accent-600)] hover:to-[var(--color-accent-700)] focus:outline-none focus:ring-2 focus:ring-offset-2 hover:cursor-pointer focus:ring-[var(--color-accent-500)] transition"
                >
                  Save Model
                </button>
              </div>
