...
const handleChange = (e) => {
  ...
  // Set selected target in form data
  setFormData(prev => ({
    ...prev,
    target_column: target
  }));

  // Extract target column values from previewData or full dataset
  const targetValues = previewData.map(row => row[target]);

  // Determine unique values
  const uniqueValues = Array.from(new Set(targetValues.map(val => String(val).trim())));

  // Binary classification check (only '0' and '1' or 0 and 1 as values)
