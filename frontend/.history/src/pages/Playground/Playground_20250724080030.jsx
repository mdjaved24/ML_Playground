...
const Playground = () => {
  ...
  const handleSaveModel = async () => {
    try {
      ...
      await axios.post(`${API_URL}/file/save/`, payload, {
        headers: {
  ...
  const renderVisualization = () => {
    ...
    if (showVisualization !== 'preview' && showVisualization !== 'stats' && !chartData) {
    ...
    const commonOptions = {
      ...
      plugins: {
        title: {
          display: true,
          text: selectedColumn2 ? `${selectedColumn} vs ${selectedColumn2}` : 
          ...
          {results && (
            <div className="bg-white rounded-xl shadow-md p-6 border border-[var(--color-gray-200)]">
              ...
              <button
                ...
                className="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-white bg-[var(--color-accent-600)] hover:bg-[var(--color-accent-700)] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[var(--color-accent-500)] transition backdrop-blur-sm cursor-pointer"
              >
                Save Model
              </button>
