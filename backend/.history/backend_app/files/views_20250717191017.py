...
class PredictionView(APIView):
    ...
    def post(self, request, pk):
        try:
            ...
            # Validate inputs
            input_data = request.data.get('inputs', {})  # Change to dictionary input
            
            # Get the expected columns from the model's config
            expected_columns = config.features
            
            # Create ordered input list matching the training columns
            features = []
            for col in expected_columns:
                if col in input_data:
                    features.append(input_data[col])
                else:
                    # Handle missing columns - either raise error or provide default
                    return Response(
                        {"error": f"Missing required feature: {col}"},
                        status=status.HTTP_400_BAD_REQUEST
                    )
            
            # Get file paths and load models
            ...
            predicted_output = load_model_and_predict(
                ...
                features=features,
                columns=expected_columns,  # Use the original training columns
                encoder=encoder,
                scaler=scaler,
