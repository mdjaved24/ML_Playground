...
class SaveModelView(APIView):
    ...
    def post(self, request):
        ...
        required_fields = ['model_cache_key','encoder_cache_key','scaler_cache_key', 'dataset', 'config']
        ...
        name = UploadedDataset.objects.get(pk=dataset).name
...
class PredictionView(APIView):
    ...
    def post(self, request, pk):
        ...
        try:
            ...
            # Ensure feature_types exists
            if not hasattr(config, 'feature_types'):
                config.feature_types = {f: 'numeric' for f in config.features}
                config.save()
            
            # Validate inputs
            input_data = serializer.validated_data['inputs']
            missing_features = [f for f in config.features if f not in input_data]
            if missing_features:
                return Response(
            ...
            # Prepare features array in EXACTLY the same order as training
            features = []
            for feature_name in config.features:
                value = input_data[feature_name]
                feature_type = config.feature_types.get(feature_name, 'numeric')
                
                try:
                    # Convert values exactly as done during training
                    if feature_type == 'categorical':
                        if isinstance(value, str):
                            ...
                            if value in ['yes', 'y', '1', 'true']:
                                ...
                                raise ValueError(f"Invalid categorical value: {value}")
                        value = int(value)
                    else:  # numeric
                        value = float(value)
                    ...
                    return Response(
                        {"error": f"Invalid value for {feature_name}: {str(e)}"},
                        status=status.HTTP_400_BAD_REQUEST
                    )
            
            # Get file paths - using .path for Django FileFields
            ...
            # Make prediction with identical preprocessing
...
def clean_parameters(params):
    ...
    for k, v in params.items():
        ...
        try:
            ...
            cleaned[k] = v  # keep strings like "gini"
    return cleaned