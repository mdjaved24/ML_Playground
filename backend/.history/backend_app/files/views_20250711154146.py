...
class PredictionView(APIView):
    ...
    def post(self, request, pk):
        try:
            ...
            try:
                features = [float(x) for x in input_data]
...
class ModelDownloadView(APIView):
    ...
    def get(self, request, pk):
        try:
            ...
            if zip_buffer.tell() == 0:  # Check if zip is empty
                return Response(
                    {"error": "No model files found to download"},
                    status=status.HTTP_404_NOT_FOUND
                )
            ...
            # Set filename in header (RFC 6266 format)
