...
import uuid
from django.db.models import Sum, Avg, Min, Max, Count, Case, When, FloatField
...
class DashboardStats(APIView):
    ...
    def get(self, request):
        ...
        performance = SavedModel.objects.filter(user=request.user).annotate(
        performance_range=Case(
            When(accuracy__gt=90, then='High (>90%)'),
            When(accuracy__gt=80, then='Good (80-90%)'),
            When(accuracy__gt=70, then='Fair (70-80%)'),
            default='Poor (<70%)',
