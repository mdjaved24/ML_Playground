        ...
        try:
            ...
            for col in columns:
                ...
                if pd.api.types.is_numeric_dtype(df[col]):
                    # Treat binary numeric columns (like 0 and 1) as categorical
    ...
    def post(self, request):
        try:
            ...
            try:
                ...
                print(f"Target Column: {df[config['target_column']].unique()}")
...
class PredictionView(APIView):
    ...
    def post(self, request, pk):
        try:
            ...
            # 2. Process input data - handle both dictionary and array formats
            ...
            # Case 1: Dictionary format (recommended)
            if 'inputs' in input_data and isinstance(input_data['inputs'], dict):
                input_dict = input_data['inputs']
                columns = config.features
                features = [input_dict.get(col) for col in columns]
                
                # Check for missing values
                if None in features:
                    missing = [col for col, val in zip(columns, features) if val is None]
                    return Response(
                        {"error": f"Missing required features: {', '.join(missing)}"},
                        status=status.HTTP_400_BAD_REQUEST
                    )
            
            # Case 2: Array format with separate columns
                ...
                if len(features) != len(columns):
                    return Response(
                        {"error": "Number of values doesn't match number of columns"},
                        status=status.HTTP_400_BAD_REQUEST
                    )
                
                # Convert to dictionary format for processing
                input_dict = dict(zip(columns, features))
                columns = config.features
                features = [input_dict.get(col) for col in columns]
            ...
            model = joblib.load(saved_model.model_file.path)
            encoder = joblib.load(saved_model.encoder_file.path) if saved_model.encoder_file else None
            scaler = joblib.load(saved_model.scaler_file.path) if saved_model.scaler_file else None
            target_encoder = joblib.load(saved_model.target_encoder.path) if saved_model.target_encoder else None
